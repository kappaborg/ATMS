    ...
    def process_video(self, display: bool = True, save_output: bool = True):
        ...
        try:
            while cap.isOpened():
                ...
                # Refresh on cadence even with ZERO detections — an empty
                # road is real input (zero demand keeps/advances the phase);
                # gating on `detections` froze the last decision forever.
                if self.decision_engine and self.frame_count % self.decision_update_interval == 0:
                    try:
                        # FIX: Add timeout to prevent hanging
